  ...
  <section class="section-container">
    <div class="flex flex-col md:flex-row text-default mx-auto container">
      ...
      <div class="flex-1 flex justify-center items-center">
        <NuxtImg src="img/about/portada-about.png" class="" />
      </div>

      <div class="flex flex-col flex-1 gap-4 p-8">
        <h1 class="text-3xl font-bold text-red-500 text-center"> {{ props.dataSection.title }} </h1>
        <h2 class="text-2xl font-bold">{{ props.dataSection.subTitle }}</h2>
        <p v-for="(paragraph, iParagraph) in props.dataSection.content" :key="iParagraph">
          {{ paragraph }}
        </p>
      </div>
